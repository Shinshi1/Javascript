...
function filtrarProductos() { //FILTRAR PRODUCTOS EN LA TABLA INGRESANDO PARTE DEL NOMBRE
    ...
    if (inputFiltrar.value !== "") {
        const resultado = listaPostres.filter(postre => postre.nombre.includes(inputFiltrar.value))
...
function agregarAlCarrito(id) {
    debugger
    const tablaCarrito = document.querySelector("#tablaCarrito") //conectamos a DOM - para que no cree un TBody nuevo
    const postre = listaPostres.find(postr => postr.id == id)
    carrito.push(postre)
...
const filtrarCategoria = () => {
    if (categoria.value !== "") {
        const resultado = listaPostres.filter(postre => postre.categoria.includes(categoria.value))
        if (resultado.length === 0) {
            console.clear()
...
cargarPostre(cPostre, listaPostres)
...
const cargarInfo = async() => {
    try {
        ...
        const data = await response.json()
                ...
                listaPostres = data
                listaPostres.forEach(etiqueta => {
                    contenidoHTML += mostrarCard(etiqueta)
                })
    } catch (error) {
